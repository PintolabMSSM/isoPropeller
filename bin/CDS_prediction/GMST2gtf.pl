...
=start
Author: ALin
Purpose: To convert the .fnn or .faa file generated by GMST to gtf format.
Change log:
	v1.1	2023-10	Adopted from CPAT_best2gtf_v1.1.pl.
	v1.2	2023-11	Fixed an issue in parsing attributes when reading a gtf file. Added input of the transcript .fa and TIS efficiency file for getting TIS efficiency. Added minimal ORF length and NMD identification. Allowed multiple entries per transcript and used various metrics, such that stop codon > start codon > TIS efficiency > ORF length > 3' UTR length. Added attribute tis_efficiency in the gtf output.
=cut
...
my ($gtf, $fa, $gmst, $out, $min_orf, $nmd, $tis, $help) = ("", "", "", "", 1, -1, "", 0);

my $usage = "Usage: perl GMST2gtf.pl 
...
my $orf_ref = parse_gmst($transcript_ref, $fa_ref, $tis_ref, $gmst, $min_orf, $nmd);
...
sub parse_gmst{
	my ($temp_transcript_ref, $temp_fa_ref, $temp_tis_ref, $temp_gmst, $temp_min_orf, $temp_nmd) = @_;
	my %temp_start_codon = ('ATG', 1);
	my %temp_stop_codon = ('TAA', 1, 'TAG', 1, 'TGA', 1);
	my $temp_seq_in = Bio::SeqIO->new(-file => $temp_gmst, -format => 'fasta',);
	my %temp_orf = ();
	while(my $temp_seq_obj = $temp_seq_in->next_seq){
		my $temp_transcript_id = $temp_seq_obj->id;
		my $temp_desc = $temp_seq_obj->desc;
		$temp_desc =~ /\|(\d+)_nt\|([+-])\|(\d+)\|(\d+)$/;
		my ($temp_orf_length, $temp_orf_strand, $temp_orf_start, $temp_orf_end) = ($1, $2, $3, $4);
		...
		#ORF: ORF strand, ORF length, 5' UTR length, 3' UTR length, canonical start codon, TIS efficiency, canonical stop codon, NMD
		my @temp_orf = ($temp_orf_strand, $temp_orf_length, $temp_orf_start - 1, $temp_transcript_ref->{$temp_transcript_id}{'length'} - $temp_orf_end, 0, 0, 0, 0);
...
sub print_result{
	...
	print $temp_table_fh "transcript_id\tlength\tcds_type\tcds_length\t5utr_length\t3utr_length\tcanonical_start\ttis_efficiency\tcanonical_stop\n";
	foreach my $temp_transcript_id (keys %$temp_orf_ref){
		...
		print $temp_table_fh "$temp_transcript_id\t$temp_transcript_ref->{$temp_transcript_id}{'length'}\t$temp_cds_type{$temp_sorted_orf[0][7]}\t$temp_sorted_orf[0][1]\t$temp_sorted_orf[0][2]\t$temp_sorted_orf[0][3]\t$temp_sorted_orf[0][4]\t$temp_sorted_orf[0][5]\t$temp_sorted_orf[0][6]\n";
		...
		EXON:foreach my $temp_exon_ref (@temp_sorted_exon){
			...
			if(($temp_orf_left >= $temp_moving_left) && ($temp_orf_right <= $temp_moving_right)){
				...
				print $temp_gtf_fh "$temp_transcript_ref->{$temp_transcript_id}{'chr'}\tGMST\tCDS\t$temp_left\t$temp_right\t.\t$temp_strand\t$temp_frame\ttranscript_id \"$temp_transcript_id\"\; gene_id \"$temp_transcript_ref->{$temp_transcript_id}{'gene_id'}\"\; tis_efficiency \"$temp_sorted_orf[0][5]\"\;\n";
				last EXON;
			}
				...
				print $temp_gtf_fh "$temp_transcript_ref->{$temp_transcript_id}{'chr'}\tGMST\tCDS\t$temp_left\t$temp_exon_ref->[1]\t.\t$temp_strand\t$temp_frame\ttranscript_id \"$temp_transcript_id\"\; gene_id \"$temp_transcript_ref->{$temp_transcript_id}{'gene_id'}\"\; tis_efficiency \"$temp_sorted_orf[0][5]\"\;\n";
				...
				print $temp_gtf_fh "$temp_transcript_ref->{$temp_transcript_id}{'chr'}\tGMST\tCDS\t$temp_exon_ref->[0]\t$temp_right\t.\t$temp_strand\t$temp_frame\ttranscript_id \"$temp_transcript_id\"\; gene_id \"$temp_transcript_ref->{$temp_transcript_id}{'gene_id'}\"\; tis_efficiency \"$temp_sorted_orf[0][5]\"\;\n";
				last EXON;
			}
			else{
				my $temp_frame = check_frame($temp_moving_orf, $temp_strand, $temp_exon_ref->[0], $temp_exon_ref->[1]);
				print $temp_gtf_fh "$temp_transcript_ref->{$temp_transcript_id}{'chr'}\tGMST\tCDS\t$temp_exon_ref->[0]\t$temp_exon_ref->[1]\t.\t$temp_strand\t$temp_frame\ttranscript_id \"$temp_transcript_id\"\; gene_id \"$temp_transcript_ref->{$temp_transcript_id}{'gene_id'}\"\; tis_efficiency \"$temp_sorted_orf[0][5]\"\;\n";
